...
def main():
    ...
    user_choice = st.image([rock_image, paper_image, scissors_image], caption=["Pedra", "Papel", "Tesoura"], width=150, use_column_width=True)
    
    if user_choice:
        user_choice_caption = user_choice.caption
        st.write(f"Você escolheu: {user_choice_caption}")
        ...
        result = determine_winner(user_choice_caption, computer_choice)
        st.write(result)
